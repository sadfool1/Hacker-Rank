...
def coprime_checker (Ai, Bj):
    ...
    for i, j in zip(factor(Ai), factor(Bj)):
        if factor_counter > 1:
                break
        elif factor(Ai)[i] in factor(Bj):
...
def main(A, B):
    ...
    for i, numberi in enumerate(A,0):
        for j, numberj in enumerate(B,0):
            
            if coprime_checker(numberi, numberj) == True:
                pass
            else:    
                tempi.append(i)
                tempj.append(j)
